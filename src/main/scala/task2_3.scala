...
object task2_3 extends App {
  ...
  */
  ...
  //calculating average bedroom and bathroom for price>5000 and review score = 10
  parquetFileDF.select(avg(col("bathrooms")), avg(col("bedrooms"))).show
  parquetFileDF.filter(col("price")>5000 and col("review_scores_value")===10)
    .select(avg(col("bathrooms")), avg(col("bedrooms")))
    .write.format("csv")
    .option("header", "true")
    .save("D:\\Spark\\truata_de_coding_challenge_v1\\out\\out_2_3.csv")
